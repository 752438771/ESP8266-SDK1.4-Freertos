...
flash_safe_get_sec_num(void)
{
    return (flash_safe_get_size_byte() / (SPI_FLASH_SEC_SIZE));
}

SpiFlashOpResult //ICACHE_FLASH_ATTR 
flash_safe_read(uint32 src_addr, uint32 *des_addr, uint32 size)
{
...
SpiFlashOpResult //ICACHE_FLASH_ATTR 
flash_safe_write(uint32 des_addr, uint32 *src_addr, uint32 size)
{
    SpiFlashOpResult result = SPI_FLASH_RESULT_ERR;
    FLASH_SAFEMODE_ENTER();
...
SpiFlashOpResult //ICACHE_FLASH_ATTR 
flash_safe_erase_sector(uint16 sec)
{
    SpiFlashOpResult result = SPI_FLASH_RESULT_ERR;
    FLASH_SAFEMODE_ENTER();
...
flash_rom_get_size_type(void)
{
    return flash_rom_getinfo().size;
}

uint32_t //ICACHE_FLASH_ATTR 
flash_rom_get_size_byte(void)
{
    static uint32_t flash_size = 0;
    if (flash_size == 0)
    {
...
flash_rom_set_size_type(uint8_t size)
{
...
bool //ICACHE_FLASH_ATTR 
flash_rom_set_size_byte(uint32_t size)
{
    // Dangerous, here are dinosaur infested!!!!!
    // Reboot required!!!
    // If you don't know what you're doing, your nodemcu may turn into stone ...
...
bool //ICACHE_FLASH_ATTR 
flash_rom_set_speed(uint32_t speed)
{
    // Dangerous, here are dinosaur infested!!!!!
    // Reboot required!!!
    // If you don't know what you're doing, your nodemcu may turn into stone ...
...
uint8_t //ICACHE_FLASH_ATTR 
byte_of_aligned_array(const uint8_t *aligned_array, uint32_t index)
{
...
uint16_t //ICACHE_FLASH_ATTR 
word_of_aligned_array(const uint16_t *aligned_array, uint32_t index)
{
    if ( (((uint32_t)aligned_array) % 4) != 0 )
    {
        FLASH_DEBUG("aligned_array is not 4-byte aligned.\n");
